...
void cmd_encoder_register(void)
{
    ...
    commands_cmd_init(&cmd_entry, "enc", "encoder application commands", cmd_help, ARRAY_SIZE(cmd_help), cmd_encoder);
...
int cmd_encoder(int argc, char **argv)
{
    ...
    if (argc == 2 && strcmp("deinit", argv[1]) == 0) {
        if (task_encoder_deinit() != SUCCESS) {
            return CMD_FAILURE;
        }
    ...
    if (argc >= 2 && strcmp("stats", argv[1]) == 0) {
        if (argc == 3 && strcmp("print", argv[2]) == 0) {
            task_encoder_stats_print();
            return CMD_SUCCESS;
        }

        if (argc == 3 && strcmp("reset", argv[2]) == 0) {
            task_encoder_stats_reset();
            return CMD_SUCCESS;
        }
    }
